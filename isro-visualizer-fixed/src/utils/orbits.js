...
// ─── Custom Mission Registry ──────────────────────────────────────────────────
// Replaces the window.__ISRO_MISSIONS__ global — avoids React batching race
// conditions where the effect fires late and leaves stale data.
// Call registerMissions(missions) from App.jsx on every missions update.
let _missionRegistry = [];
export function registerMissions(missions) { _missionRegistry = missions; }
export function getMissionRegistry()       { return _missionRegistry; }
...
// Artistic planet radii in scene units (mirrors SolarSystem.jsx ARTISTIC_RADII).
// Kept here so getMissionScenePos can nudge spacecraft off planet surfaces.
const PLANET_SCENE_RADII = { earth:0.62, mars:0.38, venus:0.58, moon:0.28, jupiter:1.80 };

/**
 * Returns a heliocentric XYZ point just above the planet's visible surface.
 * missionAngle — a per-mission constant so ships park at distinct spots.
 * liftAU       — distance above surface in scene units (converted to AU internally).
 */
function parkedOnSurface(planetHxyz, missionAngle, liftSceneUnits = 1.1) {
  const liftAU = liftSceneUnits / AU_TO_SCENE;
  return {
    x: planetHxyz.x + Math.cos(missionAngle) * liftAU,
    y: planetHxyz.y + liftAU * 0.25,
    z: planetHxyz.z + Math.sin(missionAngle) * liftAU,
  };
}
...
export function getMissionScenePos(missionId, date) {
  ...
  switch (missionId) {
    ...
    case 'chandrayaan1': {
      ...
      if (date < launch || date > end) return helioToScene(parkedOnSurface(earthHxyz, 0.3));
    ...
    case 'chandrayaan2': {
      ...
      if (date < launch) return helioToScene(parkedOnSurface(earthHxyz, 1.0));
      const geo = getLunarTransferPos(
    ...
    case 'chandrayaan3': {
      ...
      if (date < launch) return helioToScene(parkedOnSurface(earthHxyz, 1.8));
    ...
    case 'mangalyaan': {
      ...
      if (date < launch) return helioToScene(parkedOnSurface(earthHxyz, 2.6));
      if (date < moi) {
      ...
      if (date > eom) return helioToScene(parkedOnSurface(marsHxyz, 1.4));
    ...
    case 'adityaL1': {
      ...
      if (date < launch) return helioToScene(parkedOnSurface(earthHxyz, 3.4));
    ...
    case 'spadex': {
      ...
      if (date < launch) return helioToScene(parkedOnSurface(earthHxyz, 4.2));
    ...
    case 'gaganyaan': {
      ...
      if (date < launch) return helioToScene(parkedOnSurface(earthHxyz, 5.0));
    ...
    case 'lupex': {
      ...
      if (date < launch) return helioToScene(parkedOnSurface(earthHxyz, 0.7));
    ...
    case 'nisar': {
      ...
      if (date < launch) return helioToScene(parkedOnSurface(earthHxyz, 1.5));
      const SSO_AU = 0.005;
    ...
    case 'shukrayaan': {
      ...
      if (date < launch) return helioToScene(parkedOnSurface(earthHxyz, 2.3));
      if (date > arrival) {
    ...
    case 'chandrayaan4': {
      ...
      if (date < launch) return helioToScene(parkedOnSurface(earthHxyz, 3.1));
    ...
    default: {
      ...
      // Look up orbitTarget from the module-level registry (populated by App.jsx)
      const allMissions = getMissionRegistry();
